...
try:
    ...
    for university, data in scraped_data.items():
        ...
        if data["AdmissionsDetails"]:
            admission_data = [{"UniversityID": university_id, "AdmissionDetailsURL": data["AdmissionsDetails"]}]
            insert_data(cursor, "Admission", admission_data)

        # Insert Fee Structure URL
        if data["feestructure"]:
            fee_structure_data = [{"UniversityID": university_id, "FeeStructureURL": data["feestructure"]}]
